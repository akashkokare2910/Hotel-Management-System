...
Tops = Frame(root,bg='orange',bd=15,pady=4,relief=RIDGE)
Tops.pack(side=TOP)

lblTitle=Label(Tops,font=('arial',50,'bold italic'),text='WELCOME TO PIZZAHUT',bd=21,bg='black',
...
Name = Label(root,font = ('arial',19,'bold italic'),text="Name:-",fg="black")
Name.place(x=200,y=170)

mobileno = Label(root,font = ('arial',19,'bold italic'),text="Mobile No:-",fg="black")
mobileno.place(x=730,y=170)

Name = StringVar()
mobileno = IntVar()

Name=Entry(root,width=50,textvariable = Name)
Name.place(x=300,y=180)

mobileno=Entry(root,width=50, textvariable = mobileno)
mobileno.place(x=890,y=180)

#==================================textfile======================
def save_info1():
    name_info = Name.get()
    mobileno_info = mobileno.get()
    Total_info = TotalCost.get()
    print(name_info,mobileno_info,Total_info)

    file = open("test12.txt",'a')
    file.write(f"{name_info,mobileno_info,Total_info}")
    file.close()
...
btnSubmit=Button(Buttons_F,padx=16,pady=1,bd=7,fg='black',font=('arial',16,'bold'),width=4,text='Submit',
                        bg='orange',command=save_info1).grid(row=2,column=1)
