...
lblTitle = Label(Tops, font=('Comic Sans MS', 60, 'bold', 'underline'), text='SUBWAY', bd=21, bg='black',fg='cornsilk', justify=CENTER)
lblTitle.grid(row=0)
...
def save_info():
    ...
    totalbill_in = TotalCost.get()
    print(name_in, mobileno_in, totalbill_in)

    file = open("bill.txt", "a")
    file.write(f"{name_in, mobileno_in, totalbill_in}")
    # file.write(mobileno_in+ "\n")
    # file.write(totalbill_in+ "\n")

    file.close()


Name_text = Label(root, font=('Comic Sans MS', 20, 'bold'), text="NAME-", fg="black", bg='orange')
Name_text.place(x=100, y=210)

mobileno_text = Label(root, font=('Comic Sans MS', 20, 'bold'), text="PHONE NO-", fg="black", bg='orange')
mobileno_text.place(x=600, y=210)
...
Name = Entry(root, width=30, textvariable=Name)
Name.place(x=210, y=220)

mobileno = Entry(root, width=30, textvariable=mobileno)
mobileno.place(x=810, y=220)
...
def iExit():
    iExit = tkinter.messagebox.askyesno("Exit Restaurant System", "Do you want to exit!!")
    if iExit > 0:
        root.destroy()
        return
...
def CostofItem():
    ...
    PriceofDrinks = (Item1 * 65) + (Item2 * 75) + (Item3 * 99) + (Item4 * 130) + (Item5 * 180) + (Item6 * 75) + (
                Item7 * 75) + (Item8 * 89)

    PriceofFood = (Item9 * 260) + (Item10 * 175) + (Item11 * 255) + (Item12 * 480) + (Item13 * 240) + (Item14 * 110) + (
                Item15 * 340) + (Item16 * 213)
    ...
    SubTotalofITEMS = "Rs", str('%.2f' % (PriceofDrinks + PriceofFood))
    SubTotal.set(SubTotalofITEMS)

    Tax = "Rs", str('%.2f' % ((PriceofDrinks + PriceofFood + 1.59) * 0.18))
    PaidTax.set(Tax)

    TT = ((PriceofDrinks + PriceofFood + 1.59) * 0.18)
...
Water = Checkbutton(Drinks_F, text='Water', variable=var4, onvalue=1, offvalue=0, font=('arial', 18, 'bold'),
                    bg='orange', command=chk_Water).grid(row=3, sticky=W)
...
MilkShake = Checkbutton(Drinks_F, text='MilkShake', variable=var6, onvalue=1, offvalue=0, font=('arial', 18, 'bold'),
                        bg='orange', command=chk_MilkShake).grid(row=5, sticky=W)
...
txtWater = Entry(Drinks_F, font=('arial', 16, 'bold'), bd=8, width=6, justify=LEFT, state=DISABLED
                 , textvariable=E_Water)
txtWater.grid(row=3, column=1)

txtCappuccino = Entry(Drinks_F, font=('arial', 16, 'bold'), bd=8, width=6, justify=LEFT, state=DISABLED
                      , textvariable=E_Cappuccino)
txtCappuccino.grid(row=4, column=1)

txtMilkShake = Entry(Drinks_F, font=('arial', 16, 'bold'), bd=8, width=6, justify=LEFT, state=DISABLED
...
Fries = Checkbutton(Food_F, text="Fries ", variable=var14, onvalue=1, offvalue=0,
                    font=('arial', 16, 'bold'), bg='orange', command=chk_Fries).grid(row=5, sticky=W)
...
btnReceipt = Button(Buttons_F, padx=16, pady=1, bd=7, fg='black', font=('arial', 16, 'bold'), width=4, text='BILL',
...
btnSUBMIT = Button(Buttons_F, padx=16, pady=1, bd=7, fg='black', font=('arial', 16, 'bold'), width=4, text='SUBMIT',
                   bg='orange', command=save_info).grid(row=2, column=0)
